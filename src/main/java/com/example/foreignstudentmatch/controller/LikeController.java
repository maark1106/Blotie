...
public class LikeController {
    ...
    @PostMapping("/{feed_id}")
    public ResponseDto<?> addLike(@RequestParam("student_id") Long studentId, @PathVariable("feed_id") Long feedId) {
        likeService.addLike(studentId, feedId);
        return new ResponseDto<>("200", "이 피드를 좋아합니다", null);
    }

    @DeleteMapping("/{feed_id}")
    public ResponseDto<?> removeLike(@RequestParam("student_id") Long studentId, @PathVariable("feed_id") Long feedId) {
